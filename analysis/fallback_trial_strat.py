...
class TrialSequenceAnalyzer:
    ...
    def retrieve_list_trial_order(self, file_path):
        # Clean up the data
        filename_csv_start_str = (
            f"Unity_trial_data_sub{self.expt_file_info.subject_id}_"
    ...
    def plot_shift_figure(
        ...
        if (save_fig == True) and (output_figDir is not None):  # noqa: E712
            ...
            fig.savefig(
                output_figDir
                + "/Shift_MOCS_PresentationOrder_"
                + f"simulatedTask_wMOCS_AEPsych_sub{self.expt_file_info.subject_id}_"
                + f"session{self.expt_file_info.session_num}.pdf",
                bbox_inches="tight",
            )
    ...
    def plot_presentation_order_heatmap(self, save_fig=False, output_figDir=None):
        ...
        ax_side1.plot(
            initial_nMOCS_perSession - self.nTrials_perBlock // 2,
            list(range(self.nBlocks))[::-1],
            color="k",
        )
        ax_side2.plot(
            final_nMOCS_perSession - self.nTrials_perBlock // 2,
            list(range(self.nBlocks))[::-1],
            color="k",
        )
        ...
        self._plot_heatmap(
            ax_main1,
            binary_initial_seq,
            "Initially pre-generated trial sequence (Gray: MOCS)",
            yticklbl,
        )
        self._plot_heatmap(
            ax_main2,
            binary_final_seq,
            "Final trial sequence after dynamic updates (Gray: MOCS)",
            yticklbl,
            xticklbl,
        )
        ...
        if (save_fig == True) and (output_figDir is not None):  # noqa: E712
            ...
            fig4.savefig(
                output_figDir
                + "/Pattern_PresentationOrder_"
                + f"simulatedTask_wMOCS_AEPsych_sub{self.expt_file_info.subject_id}_"
                + f"session{self.session_today}.pdf",
                bbox_inches="tight",
            )
