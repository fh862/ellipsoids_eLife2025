...
from analysis.ellipses_tools import (
    compute_radii_scaler_to_reach_targetPC,
    covMat_to_ellParamsQ,
    distance_to_ellipse_boundary,
    rotate_relocate_stretched_ellipse,
    stretch_unit_circle,
)
    ...
    def _unpack_config(self):
        ...
        space_mappings = {
            "R": dict(
                ndims=2,
                fixed_color_dim=0,
                varying_color_dim=[1, 2],
                stim_space="GB plane",
            ),
            "G": dict(
                ndims=2,
                fixed_color_dim=1,
                varying_color_dim=[0, 2],
                stim_space="RB plane",
            ),
            "B": dict(
                ndims=2,
                fixed_color_dim=2,
                varying_color_dim=[0, 1],
                stim_space="RG plane",
            ),
            "lum": dict(
                ndims=2,
            ...
            "": dict(
                ndims=3,
                fixed_color_dim=None,
                varying_color_dim=[0, 1, 2],
                stim_space="cube",
            ),
        }
...
class TrialPlacement_sobolRef(NonAdaptiveTrialPlacement):
    ...
    def _sobol_generate_ref(self):
        ...
        """
        sobolRef_cat = sim_MOCS_trials.sample_sobol(
            self.config.nSims,
        ...
        for i in range(self.config.nSims):
            ...
            if self.config.stim_space == "Isoluminant plane":
                ...
                *_, stim_thres = sim_CIELab.find_threshold_point_on_isoluminant_plane(
                ...
                opt_vecLen = sim_CIELab.find_vecLen(
                    ref_i,
...
class TrialPlacement_gridRef(TrialPlacement_sobolRef):
    ...
    def _random_points_on_unit_sphere(self):
        ...
        randx, randy, randz = (
            randx_noNoise + noise_x,
            randy_noNoise + noise_y,
            randz_noNoise + noise_z,
        )
