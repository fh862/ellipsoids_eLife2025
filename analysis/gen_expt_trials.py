...
class ExptTrialGeneration(SimulateTrialGivenWishart):
    ...
    def __init__(
        ...
        expt_dim,
        config_all,
        flag_noMore_MOCS=False,
        max_shifts_MOCS=4,
        gt_Wishart=None,
        ref=None,
        pseudo_randomize=False,
        pseudo_randomize_seed=None,
        val_scaler=None,
        customized_val_scaler=None,
        communicator=None,
        color_thres=None,
    ):
        """
    ...
    # %%
    def _monitor_time_insert_MOCS_trials(
        self,
        start_time,
        max_wait_time,
        trial_sequence,
    ...
    def _monitor_time_insert_pregenSobol_trials(
        self,
        start_time,
        max_wait_time,
        trial_sequence,
        ...
        while not stop_event.is_set():  # Exit if stop_event is set
            ...
            if elapsed_time > max_wait_time_ii:
                ...
                # Send the stimulus information and wait for the subject to make a response
                binaryResp = self._send_stim_and_wait_for_resp(
                    trial_sequence.nTrials_total + sobol_idx,
                    f"Sobol_{sobol_idx}",
                    xref,
                    x1,
                )
                ...
                if trial_type == "MOCS":
                    ...
                    if (not pause_for_bumpingMOCS) and (not self.flag_noMore_MOCS):
                        ...
                        monitor_thread = threading.Thread(
                            target=self._monitor_time_insert_MOCS_trials,
                            args=(
                                start_time,
                                max_wait_time,
                                trial_sequence,
                        ...
                        # Start monitoring trial generation in a separate thread
                        monitor_thread = threading.Thread(
                            target=self._monitor_time_insert_pregenSobol_trials,
                            args=(
                                start_time,
                                max_wait_time,
                                trial_sequence,
...
class LoadExptInfo:
    ...
    def load_pregenerated_MOCS(file_path, session_today, nTrials_MOCS_perSession):
        ...
        idxTrial_MOCS = [
            (session_today - 1) * nTrials_MOCS_perSession,
    ...
    def load_pregenerated_val_scaler(file_path, session_today, nTrials_AEPsych_perSession):
        ...
        idxTrial_AEPsych = [
            (session_today - 1) * nTrials_AEPsych_perSession,
