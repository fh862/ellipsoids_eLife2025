...
class CrossValidation:
    ...
    def select_NFold_data_noFixedRef(data, total_folds):
        ...
        if row_ub_list[-1] != nTrials_total:
            raise ValueError(
                f"Index computation error: Final upper index {row_ub_list[-1]}"
...
class TrialDistribution:
    def separate_edge_vs_central_trials_2Dplane(
        ...
        data_central = (
            y_all[indices_central],
            xref_all[indices_central],
            x1_all[indices_central],
        )
    ...
    def truncate_edge_vs_central_trials_given_percentage(
        data,
        dim1_bd,
        ...
        vars_dict = {}
        for k in [
            "edge_percentage_orgedge_percentage_desired",
            "data_edge_trunc",
            "data_central_trunc",
            "data_edge",
            "data_central",
    ...
    def load_trial_efficiency_results(flag_btst=[False], btst_seed=[None]):
        ...
        try:
            ...
            for path in full_path_set:
                ...
                if match:
                    nTrials_inclusion.append(int(match.group(1)))
            if not nTrials_inclusion:
                raise ValueError("No trial numbers found in file names.")
