...
class wishart_model_pred_inCIELab(wishart_model_pred):
    ...
    def convert_Sig_2DisothresholdContour_oddity_batch_inLab(
        self, Lab_ref, CLabTool, ell_vis_scaler=1, flag_compute_thres_fd=True
    ):
        """
            ...
            for j in range(self.num_grid_pts2):
                ...
                *_, Lab_comp_rep_ij = self.setup_bruteforce_comp(Lab_ref_ij, CLabTool)
                ...
                for c in range(self.params["n_theta"]):
                    ...
                    (
                        self.Lab_thres_unscaled_CLab[i, j, :, c],
                        _,
                        self.deltaE[i, j, c],
                ...
                vd = self.cfg.varied_dims
                (
                    fitEll_scaled_CLab_ij,
                    fitEll_unscaled_CLab_ij,
                    self.params_ell_CLab[i][j],
                    Lab_thres_scaled_CLab_ij,
                ) = fit_2d_isothreshold_contour(
