...
class SimThresCIELab:
    ...
    def get_planes(self, grid_lb, grid_ub, num_grid_pts=5, fixed_val=0.5):
        ...
        for i, plane_str in enumerate(self.plane_2D_list):
            plane_3slices[i], grid_1d, X, Y = self.get_plane_1slice(
                grid_lb, grid_ub, num_grid_pts, fixed_val, plane_2D=plane_str
            )
    ...
    def find_threshold_point_on_isoluminant_plane(
        self, W_ref, chrom_dir, M_RGBTo2DW, M_2DWToRGB, deltaE, coloralg="CIE1994"
    ):
        """
    ...
    def find_boundary_LabPts_onSlice(
        self,
        fixedVal,
        fixedDim,
        rgb_bds=[0, 1],
