...
def shuffle_sobol_trials(
    ...
    if shuffle_val_scaler_max_strat >= len(val_scaler):
        raise ValueError(
            f"Invalid value: shuffle_val_scaler_max_strat cannot exceed {len(val_scaler) - 1}. "
            "No scalers will be shuffled."
        )
    ...
    val_scaler_all = np.repeat(
        val_scaler[:shuffle_val_scaler_max_strat],
