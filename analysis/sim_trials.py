...
class SimulateTrialGivenWishart:
    ...
    def _create_pseudorandom_order(self):
        ...
        rng = (
            np.random.default_rng(self.pseudo_randomize_seed)
            if self.pseudo_randomize_seed is not None
    ...
    def _configure_trial(self, client, trial_counter, config_index):
        ...
        if trial_counter == 0:
            client.configure(
                config_str=self.config_all[config_index],
            ...
            for i in range(self.numConfig):
                ...
                if flag_insert_pregen:
                    ...
                    monitor_thread = threading.Thread(
                        target=self._monitor_time_insert_pregenerated_trials,
                        args=(
                            start_time,
                            max_wait_time,
                            pregenerated_trials,
