...
class fit_PMF_MOCS_trials:
    def __init__(
        ...
        resp: np.ndarray,
        nLevels: int,
        guess_rate: float = 1 / 3,
        target_pC: float = 0.667,
        ...
        for n in range(nLevels):
            ...
            if key_pair in self._geodesic_path_dict or key_pair in self._geodesic_dist_dict:
                ...
                state, metrics = geodesics.estimate_v0(
                    ...
                    self.geo_key,
                    num_generations=self.geo_num_gen,
                    popsize=self.geo_pop_size,
                    tolerance=self.geo_tol,
...
# %%
def compute_Wishart_based_pCorrect_atMOCS(
    numBtst, nLevels, fit_PMF_MOCS, xref_unique, model_pred, color_thres_data, ndims=2
):
    """
    ...
    for n in trange(nRefs):
        ...
        pChoosingX1_Wishart[n] = model_pred._compute_pChoosingX1(
            np.full(finer_stim.shape, 0) + xref_unique[n], finer_stim + xref_unique[n]
        )
