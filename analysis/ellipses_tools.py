...
def angles_to_2Dchromatic_directions(theta_deg, normalize=True):
    ...
    if normalize:
        ...
        if not np.all(valid):
            n_bad = np.sum(~valid)
            print(
                f"[angles_to_chromatic_directions_2D] Warning: {n_bad} direction(s) "
...
# %%
def fit_2d_isothreshold_contour(
    ref,
    comp,
    nTheta=200,
    ellipse_scaler=1.0,
...
def compute_radii_scaler_to_reach_targetPC(
    pC_target, ndims=2, lb=2, ub=3, nsteps=100, nz=int(1e4), flag_visualize=False
):
    """
