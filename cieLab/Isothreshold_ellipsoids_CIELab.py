...
from analysis.ellipsoids_tools import (
    UnitCircleGenerate_3D,
...
for idx in tqdm(np.ndindex(*base_shape1), total=np.prod(base_shape1), desc="Computing"):
    ...
    for d in np.ndindex(*base_shape2):
        ...
        opt_vecLen[*idx, *d] = sim_thres_CIELab.find_vecLen(
            rgb_ref_idx, vecDir, deltaE_1JND, coloralg=color_diff_algorithm
        )
    ...
    rgb_comp_surface_unscaled[idx] = np.reshape(
        grid_xyz * opt_vecLen[idx][..., None] + rgb_ref_idx[None, None], (-1, ndims)
    ).T

    # Fit an ellipsoid to the discrete threshold points and (optionally) rescale it about ref
    (
        fitEllipsoid_scaled[idx],
        fitEllipsoid_unscaled[idx],
        ellParams[idx],
        rgb_comp_surface_scaled[idx],
    ) = fit_3d_isothreshold_ellipsoid(
        rgb_ref_idx,
...
results_keys = [
    ...
    "rgb_comp_surface_scaled",
    "rgb_comp_surface_unscaled",
    "ellParams",
    "scaler",
]
results = {}
