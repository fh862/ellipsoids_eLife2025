...
from analysis.ellipses_tools import UnitCircleGenerate, fit_2d_isothreshold_contour
...
def run_one_setting(fixed_RGBvec, nGridPts_ref, color_diff_algorithm):
    ...
    plane_points, *_ = sim_thres_CIELab.get_planes(
        lb_RGB_fine, ub_RGB_fine, num_grid_pts=nGridPts_ref_fine, fixed_val=fixed_RGBvec
    )

    # Build coarse reference grid used for threshold computations
    # ref_points shape: (nPlanes, 3, nGridPts_ref, nGridPts_ref)
    ref_points, grid_ref, X, Y = sim_thres_CIELab.get_planes(
        lb_RGB_grid, ub_RGB_grid, num_grid_pts=nGridPts_ref, fixed_val=fixed_RGBvec
    )
    ...
    rgb_comp_contour_unscaled = np.full(
        ssize
        + (
            ndims,
            numDirPts,
    ...
    rgb_comp_contour_scaled = np.full(
        ssize
        + (
            ndims,
            numDirPts,
    ...
    for p in range(sim_thres_CIELab.nPlanes):
        ...
        for ij in np.ndindex(base_size):
            ...
            for k in range(numDirPts):
                ...
                opt_vecLen[p, *ij, k] = sim_thres_CIELab.find_vecLen(
                    rgb_ref_pij, vecDir, deltaE_1JND, coloralg=color_diff_algorithm
                )
            ...
            # fit an ellipse
            (
                fitEllipse_scaled[p, *ij],
                fitEllipse_unscaled[p, *ij],
                ellParams[p, *ij],
                rgb_comp_contour_scaled[p, *ij],
            ) = fit_2d_isothreshold_contour(
                rgb_ref_pij[idx_varyingDim],
                rgb_comp_contour_unscaled[p, *ij],
                nTheta=nTheta,
    ...
    sim_CIE_vis.plot_2D_all_planes(
        grid_est,
        fitEllipse_scaled,
    ...
    results_keys = [
        ...
        "fitEllipse_scaled",
        "fitEllipse_unscaled",
        "rgb_comp_contour_scaled",
        "rgb_comp_contour_unscaled",
        "ellParams",
        "scaler",
    ]
    results = {}
