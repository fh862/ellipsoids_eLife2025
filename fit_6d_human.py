...
from analysis.utils_load import load_expt_data, select_file_and_get_path
...
if flag_running_on_hpc:
    ...
    session_files, session_file_name_part1 = load_expt_data.get_all_sessions_file_names(
        subN, nSessions, path_str, exptCond="_6dExpt_RGBcube"
    )
...
if not flag_running_on_hpc and not flag_btst:
    ...
    for i, (lb_i, ub_i) in enumerate(zip(slc_datapoints_to_show_lb, slc_datapoints_to_show_ub)):
        ...
        fig = plt.figure(
            figsize=pltSampSettings.fig_size,
            dpi=pltSampSettings.dpi,
            constrained_layout=True,
        )
        ax = fig.add_subplot(111, projection="3d")
        sampling_vis_az = SamplingRefCompPairVisualization(
        ...
        sampling_vis_az.plot_sampling(
            xref_jnp[lb_i:ub_i],  # Reference points up to the nth data point
    ...
    # Visualize the trials up to the nth data point with specified marker transparency.
    sampling_vis_az.plot_sampling(
        xref_jnp[non_edge_non_sobol_idx],  # Reference points up to the nth data point
    ...
    out_html = os.path.join(
        html_dir,
        "Ellipsoids_Fitted_ColorDiscrimination_"
