...
from plotting.visualize_MOCS import MOCSConditionsVisualization, PlotCondSettings
...
if stim_dims == 2:
    ...
    if flag_gt_CIE:
        ...
        color_thres_data = color_thresholds(stim_dims, baseDir, plane_2D=plane_2D)
...
output_figDir_sims = os.path.join(
    baseDir,
    "ELPS_analysis",
    "Simulation_FigFiles",
    "Python_version",
...
for idx_slc in range(nRefs):
    ...
    finer_stim = sim_MOCS_trials.create_discrete_stim(
        cdir_idx, nVec_len, startpoint=np.full((stim_dims,), 0), ndims=stim_dims
    )
    ...
    comp_unique_temp = sim_MOCS_trials.create_discrete_stim(
        vecVal_vecLen_t_pC,
        nLevels_wExtra,
...
if flag_debugplots:
    ...
    for idx_starting in range(trials_per_level):
        ...
        plt.scatter(
            MOCS_xref_shuffled[tt_lb:tt_ub, 0],
            MOCS_xref_shuffled[tt_lb:tt_ub, 1],
            alpha=0.5,
        )
        plt.scatter(
            MOCS_x1_shuffled[tt_lb:tt_ub, 0],
            MOCS_x1_shuffled[tt_lb:tt_ub, 1],
            alpha=0.5,
        )
        plt.plot(
            [MOCS_xref_shuffled[tt_lb:tt_ub, 0], MOCS_x1_shuffled[tt_lb:tt_ub, 0]],
            [MOCS_xref_shuffled[tt_lb:tt_ub, 1], MOCS_x1_shuffled[tt_lb:tt_ub, 1]],
            c="k",
        )
        plt.xlim([-1, 1])
        plt.ylim([-1, 1])
...
for n in range(nSessions):
    ...
    Sobol_x1[n] = (
        Sobol_xref[n] + sobol_scaler_n[:, np.newaxis] * Sobol_samples[:, 2:]
    )  # Apply scaling to last two dimensions
    ...
    if flag_debugplots:
        ...
        for m in range(nTrials_sobol_perSession):
            plt.plot(
                [Sobol_xref[n, m, 0], Sobol_x1[n, m, 0]],
