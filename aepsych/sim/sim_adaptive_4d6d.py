...
# define the server!
db_file_name = (
    f"Sim{psyfield_dims}dTask_colorDiscrimination_"
...
# a binary response.
AEPsych_trial_given_Wishart_gt = SimulateTrialGivenWishart(
    psyfield_dims, config_all, gt_Wishart, val_scaler=SOBOL_SCALER
)
...
data_vis_AEPsych = expt_data(
    ...
    server._strats[-1].x[:, 0:stim_dims] + server._strats[-1].x[:, stim_dims:],
    server._strats[-1].y,
    None,
)
...
for i, (lb_i, ub_i) in enumerate(zip(slc_datapoints_to_show_lb, slc_datapoints_to_show_ub)):
    ...
    sampling_vis.plot_sampling(
        data_vis_AEPsych.xref_all[lb_i:ub_i],  # Reference points
