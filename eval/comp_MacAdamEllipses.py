...
from analysis.conf_interval import (
    ...
    find_inner_outer_contours_nonellipse,
)
...
(
    ...
    angles_deg,
    n_ellipses,
) = load_util_files.load_MacAdam_ellipses(macadam_path)
...
if flag_running_on_hpc:
    ...
    for i in range(nBtst + 1):
        ...
        ellipses_thres_W_scaled = (
            model_pred_Wishart_MacAdam.fitEll_unscaled - grid_MacAdam[..., None]
        ) * W_scaler + grid_MacAdam[..., None]
        ...
        # Step 1: W → RGB
        edge_RGB = color_thres_data.W2D_to_rgb(
            [
                [-1, -1],  # 4 vertices of the Wishart square
                [-1, 1],
                [1, 1],
                [1, -1],
            ]
        ...
        ell_thres_W = np.concatenate(
            (
                ...
                np.ones((nRefs, 1, n_ellipse_pts_W)),
            ),
            axis=1,
        )
...
if not flag_running_on_hpc:
    ...
    if flag_plot_data:
        for n in range(nRefs):
            ...
            cie_ax.plot(
                *ell_thres_xyY[n],
                color=ref_rgb[n] if not flag_diagram_color else "k",
                lw=1,
            )
    # plot the major axis
    if flag_plot_major:
    ...
    if flag_save_fig:
        ...
        fig_output_dir = os.path.join(
            fileDir_prefix.replace("DataFiles", "FigFiles"),
            f"sub{subN}",
            "MacAdam_ellipses",
        )
        os.makedirs(fig_output_dir, exist_ok=True)
        fig_full_path = os.path.join(
            fig_output_dir,
            f"MacAdam_Ellipses_CIE1931_sub{subN}"
