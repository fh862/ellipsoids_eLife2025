...
output_figDir = os.path.join(
    os.path.dirname(os.path.join(fits_path.replace("DataFiles", "FigFiles"))),
...
for n in tqdm(range(nRefs), desc="Bootstrapping Progress"):
    ...
    # call the class
    fit_PMF_MOCS_exptN = fit_PMF_MOCS_trials(
        stim_dims,
        xy_coords_n_wOrigin,
        responses_n_wOrigin,
...
for n in range(nRefs):
    ...
    fig_name_n = (
        f"{fig_name_part1}Ref{n}_Wdim1_{np.round(xref_unique[n][0], 2)}_"
...
predComp_settings = replace(
    ...
    cmap=cmap_allref,
    fig_name=f"{fig_name_part1}_comparison_btw_MOCS" + "_WishartPredictions.pdf",
)

vis_MOCS.plot_comparison_thres(
    thres_Wishart=vecLen_at_targetPC_Wishart,
...
output_file = (
    f"Fitted_weibull_psychometric_func_Isoluminant plane_{nTrials}totalTrials"
...
variable_names = [
    ...
    "vecLen_at_targetPC_MOCS",
    "vecLen_at_targetPC_MOCS_btst",
    "stim_at_targetPC_MOCS",
    "stim_at_targetPC_Wishart",
    "Sigmas_est_xref_unique",
