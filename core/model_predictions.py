...
class wishart_model_pred:
    ...
    def _init_model_pred_list(self):
        ...
        if self.ndims_cov == 2:
            ...
            self.fitEll_scaled = np.full(
                ...
                + (
                    self.ndims_cov,
                    self.params["nPhi"] * self.params["nTheta"],
                ),  # dense points on ellipsoid
                np.nan,
            )
    ...
    def _convert_Sig_2DisothresholdContour_oddity(self, w_ref, vecLength_test):
        ...
        fitEll_scaled, fitEll_unscaled, fitEll_params, _ = fit_2d_isothreshold_contour(
...
# %%
def rerun_model_pred_wExisting_model(
    grid,
    model_pred,
    color_thres_data,
    ...
    model_pred_new = wishart_model_pred(
        model,
        model_pred.opt_params,
