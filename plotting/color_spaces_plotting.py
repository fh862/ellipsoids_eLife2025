...
class PlotGegenfurtner(PlotSettingsBase):
    ...
    G_ax_ub: int = 10
    ref_lc: str | np.ndarray | list[float] = "k"  # 'k' or RGB triplet
    ref_marker: str = "+"
    ref_ms: float = 50
    ref_lw: float = 1
...
class DKLRelatedSpacesVisualization(PlottingTools):
    ...
    def plot_Gegenfurtner_Wishart_space(
        ...
        ax.plot(
            *self.modelpred.fitEll_unscaled[idx_row, idx_col],
            c="grey",
            lw=settings.modelpred_lw,
        )
    ...
    def plot_sDKL_space(self, settings: PlotGegenfurtner, ax=None, flag_plot_ref=True):
        ...
        DKLRelatedSpacesVisualization.set_square_axis(
            ax, settings.G_ax_ub, np.linspace(-settings.G_ax_ub, settings.G_ax_ub, 5)
        )
