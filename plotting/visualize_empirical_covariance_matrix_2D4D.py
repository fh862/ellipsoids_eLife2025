...
xgrid_fine = jnp.stack(
    jnp.meshgrid(*[xgrid_dim1_fine for _ in range(ndims)]), axis=-1
...
cmap_ell = np.reshape(
    color_thres_data.W2D_to_rgb(grid_reshape), (ngrid_pts, ngrid_pts, -1)
)  # shape: (ngrid_pts, ngrid_pts, 3)
...
fig, ax, ax_ell = visualize_sigma2D.plot_2D_covMat(
    grid_new, Sigmas_noise_grid_fine, Sigmas_noise_grid_new, settings=pltCovSettings
)
...
fig.savefig(
    os.path.join(fig_outputDir, figname + figname_ext + ".pdf"),
    format="pdf",
    bbox_inches="tight",
)
