...
class PlotPMFSettings(PlotSettingsBase):
    ...
    cmap_PMF: str | np.ndarray | list[float] = field(default_factory=lambda: [0, 0, 0])
    cmap_dots: str | np.ndarray | list[float] = field(default_factory=lambda: [0, 0, 0])
    filler_pts: np.ndarray | None = None
    num_xticks: int = 4
...
class PlotThresCompSettings(PlotSettingsBase):
    ...
    cmap: np.ndarray | list[float] | None = None
...
class PlotCondSettings(PlotSettingsBase):
    ...
    comp_marker: str = "."
    comp_mc: str | np.ndarray | list[float] = "k"
    comp_lc: str | np.ndarray | list[float] = "grey"
    comp_lw: float = 0.4
    ...
    catch_ms: int = 10
    catch_ec: str | np.ndarray | list[float] = "r"
...
class MOCSTrialsVisualization(PlottingTools):
    ...
    def plot_PMF(
        self,
        slc_idx,
        settings: PlotPMFSettings,
        xref=None,
        pX1_Wishart_slc=None,
        pX1_indv_slc=None,
        ax=None,
    ):
        """
    ...
    def plot_comparison_thres(
        self,
        thres_Wishart,
        slope_org,
        slope_CI,
        xref_unique,
        settings: PlotThresCompSettings,
        ax=None,
        thres_Wishart_95btstErr=None,
        corr_coef_org=None,
        corr_coef_CI=None,
        num_overlaps=None,
    ):
        ...
        ax.fill_between(
            settings.bds,
            settings.bds * slope_CI[0],
            settings.bds * slope_CI[1],
            color=settings.line_fit_CI_lc,
            alpha=settings.line_fit_CI_alpha,
            label=settings.line_fit_CI_label,
        )
        ax.plot(
            settings.bds,
            settings.bds * slope_org,
...
class MOCSConditionsVisualization(PlottingTools):
    ...
    def plot_MOCS_conditions(
        self,
        ndims,
        xref_unique,
        comp_unique,
        color_thres_data,
        settings: PlotCondSettings,
        ax=None,
    ):
        """
        ...
        if ndims == 2:
            ...
            for idx_slc in range(len(xref_unique)):
                ...
                ax.plot(
                    ...
                    solid_capstyle="butt",
                    lw=settings.comp_lw,
                    color=settings.comp_lc,
                )
                if settings.easyTrials_highlight:
                    ax.scatter(
                        *comp[-1, :],
                        marker=settings.catch_marker,
                        facecolor="none",
                        edgecolor=settings.catch_ec,
                        alpha=settings.catch_alpha,
                        s=settings.catch_ms,
