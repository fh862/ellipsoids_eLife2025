...
class PlotTransformationSettings(PlotSettingsBase):
    ...
    facecolor_no: str | np.ndarray | list[float] = field(
...
class TrialPlacementVisualization(PlottingTools):
    ...
    def plot_transformation(
        ...
        resp,
        gt,
        settings=PlotTransformationSettings,
        ax=None,
    ):
    ...
    def plot_2D_sampledComp(self, settings: Plot2DSampledCompSettings):
        ...
        for i in range(nGrid_x):
            for j in range(nGrid_y):
                ...
                if i == 0 and j == nGrid_y // 2:
                    ax_idx.set_xlabel(settings.xlabel)
                if i == nGrid_x // 2 and j == 0:
                    ax_idx.set_ylabel(settings.ylabel)

                if j == 0:
                    ax_idx.set_yticks([grid_ref_y[i]])
                else:
                    ax_idx.set_yticks([])

                if i == 0:
                    ax_idx.set_xticks([grid_ref_x[j]])
                else:
                    ax_idx.set_xticks([])
    ...
    @staticmethod
    def plot_3D_sampledComp(
        ref_points,
        fitEllipsoid_unscaled,
        sampledComp,
        fixedPlane,
        fixedPlaneVal,
        settings: Plot3DSampledCompSettings,
        save_fig=False,
    ):
