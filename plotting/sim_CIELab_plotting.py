...
class PlotRGBToLABSettings(PlotSettingsBase):
    ...
    lab_xylim: list[float] | None = None
    lab_zlim: list[float] | None = None
...
class Plot2DSinglePlaneSettings(PlotSettingsBase):
    ...
    visualize_raw_data: bool = False
    rgb_background: list[float] | None = None
...
class Plot3DSettings(PlotSettingsBase):
    ...
    threshold_points: np.ndarray | None = None
    ref_color: list[float] | None = None
    ref_ms: float = 10
    ref_lw: float = 0.5
    surf_color: list[float] | None = None
    surf_alpha: float = 0.5
    scatter_color: list[float] | None = None
...
class PlotLabSettings(PlotSettingsBase):
    ...
    ylabel: str = "b*"
    zlabel: str = "L*"
    title: str = "L*a*b* within monitors gamut"
...
class CIELabVisualization(PlottingTools):
    ...
    def plot_primaries(self, settings: PlotPrimariesSettings, rgb=None, ax=None):
        ...
        if rgb is not None:
            for j in range(rgb.shape[1]):
                ...
                ax.plot(spd_j, c="k", linestyle=settings.ls[j], lw=settings.lw)
    ...
    def plot_RGB_to_LAB(self, ref_rgb, ref_lab, settings: PlotRGBToLABSettings, ax=None):
        ...
        ax[1].set_ylabel("b")
        ax[1].set_zlabel("L")
        ax[1].set_xticks(settings.lab_ticks)
        ax[1].set_yticks(settings.lab_ticks)
    ...
    def plot_2D_all_planes(
        self,
        grid_est,
        fitEllipse,
        settings: Plot2DSinglePlaneSettings,
        rawData=None,
        ax=None,
    ):
        """
        ...
        for i in range(nRef):
            ...
            if settings.visualize_ellipsoids:
                ...
                if nPhi * nTheta != ell_i.shape[-1]:
                    raise ValueError(
                        "The size of grid points (nPhi x nTheta) does not"
    ...
    def plot_Lab_inGamut_flattened(self, Lab, rgb, fixed_dim, settings: Plot3DSettings, ax=None):
        ...
        if fixed_dim == 0:  # fix L*, plot (a*, b*)
            ...
            ax.set_xlim(settings.zlim)
            ax.set_ylim(settings.ylim)
            ax.set_xticks(settings.zticks)
            ax.set_yticks(settings.yticks)
        else:  # fixed_dim == 2: fix b*, plot (L*, a*)
            xlbl, ylbl = settings.zlabel, settings.xlabel
            ax.set_xlim(settings.zlim)
            ax.set_ylim(settings.xlim)
            ax.set_xticks(settings.zticks)
            ax.set_yticks(settings.xticks)
    ...
    # %%
    @staticmethod
    def visualize_stimuli_at_thres(
        s_rgb,
        settings: PlotStimAtThresSettings,
        ax=None,
        label_rgb=True,
        save_fig=False,
    ):
        ...
        if ax is None:
            fig, ax = plt.subplots(
                1,
                n,
                figsize=(n * settings.fig_size_for1[0], settings.fig_size_for1[1]),
                dpi=settings.dpi,
            )
        else:
            fig = ax.figure
        ...
        for i in range(n):
            ...
            if label_rgb:
                ...
                ax[i].set_title(
                    f"[R, G, B]: [{rgb_int[0]}, {rgb_int[1]}, {rgb_int[2]}]",
