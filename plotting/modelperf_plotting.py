...
class PltBaselineSettings:
    linecolor: tuple[float, float, float] | str = "k"
    linestyle: str = "-"
    linewidth: float = 2
    linealpha: float = 0.8
...
class PltSimSettings_2D(PlotSettingsBase):
    y_ub: float = 25.0
    legend_labels: list[str | None] = field(default_factory=list)
    legend_title: str = ""
    cmap: list[Any] = field(default_factory=list)
    alpha: float = 0.6
    edgecolor: tuple[float, float, float] | str = "w"
    linestyle: str = "--"
    linewidth: float = 1
    cmap_BW: np.ndarray | None = None
    x_ub_BW: float = 0.14
    x_ub_LU: float = 3.5
    nBins_curves: int = 11
    nBins_hist: int = 33
...
class ModelPerformanceVisualization(PlottingTools):
    ...
    def plot_BWD_indvEll_vs_Wishart(
        self,
        nConds_indvEll,
        BWD_median_indvEll,
        yerr_indvEll,
        ...
        for i in range(nConds_indvEll):
            ax.errorbar(
                i + 1,
                BWD_median_indvEll[i],
                yerr=yerr_indvEll[:, i][:, np.newaxis],
                capsize=settings.errorbar_cs,
                c=settings.errorbar_c,
                marker=settings.errorbar_m,
                markersize=settings.errorbar_ms,
                lw=settings.errorbar_lw,
            )
        for i in range(nConds_Wishart):
            ax.errorbar(
                -i - 1,
                BWD_median_Wishart[i],
                yerr=yerr_Wishart[:, i][:, np.newaxis],
                capsize=settings.errorbar_cs,
                c=settings.errorbar_c,
                marker=settings.errorbar_m,
                markersize=settings.errorbar_ms,
        ...
        ax.plot(
            [0, 0],
            [0, settings.y_ub],
            ls=settings.dashed_ls,
            lw=settings.dashed_lw,
            c=settings.dashed_lc,
        )
    ...
    def _plot_baseline_hist(self, ax, bin_edges, settings: PltBaselineHistSettings):
        ...
        for m in range(nSets):
            if len(settings.cmap) == 0:
                cmap_m = np.random.rand(1, 3)
            else:
                cmap_m = settings.cmap[m]
            if len(settings.ls) == 0:
                ls = "--"
            else:
                ls = settings.ls[m]
            if len(settings.ls_median) == 0:
                ls_m = "-"
            else:
    ...
    def plot_BWD_hist(self, bin_edges, settings: PltSimSettings_2D, ax=None):
        ...
        for j in range(nSets):
            if len(settings.cmap) == 0:
                cmap_l = np.random.rand(1, 3)
            else:
                cmap_l = settings.cmap[j]
            ax.hist(
                self.model_perf.BW_distance[j].flatten(),
                bins=bin_edges,
                color=cmap_l,
...
class NFoldsCrossValidationVisualization(PlottingTools):
    ...
    def plot_nLL_varying_hyper_param(
        self,
        hyper_param,
        nLL_training_avg,
        nLL_test_avg,
        ...
        ax.plot(
            hyper_param,
            nLL_training_avg,
            color=settings.training_color,
            marker=settings.marker,
            markersize=settings.ms,
...
class EvaluateTrialEfficiencyVisualization(PlottingTools):
    ...
    def plot_BWD_varying_nTrials(
        ...
        ax.plot(
            ...
            linestyle="None",
            marker="o",
            markerfacecolor=settings.lc,
            markeredgecolor=settings.lc,
