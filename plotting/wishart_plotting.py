...
class PlotCovMatSettings(PlotSettingsBase):
    ...
    cmap_ell: str | np.ndarray = "k"
    heatmap_title_list: list[list[str]] = field(
    ...
    def _update_axes_labels(
        ...
        if ndims == 3:
            ...
            if ~np.any(np.array(unit_true_z)):
                ...
                ax.set_zticklabels(
                    [f"{x:.2f}" for x in unit_show_z[::nsteps]],
...
class WishartModelBasicsVisualization(PlottingTools):
        ...
        for l in range(nbins):
            ...
            for i in range(ndim1):
                for j in range(ndim2):
                    ...
                    self._update_axes_labels(ax[i, j], [], [], unit_true_z=[], ndims=3)
                    self._update_axes_limits(ax[i, j], settings.xyzlim, ndims=3)
    ...
    def plot_W_all(self, W, basis_orders, settings: PlotWAllSettings, ax=None):
        ...
        if settings.fig_dir and self.save_fig:
            self._save_figure(
                fig,
                f"{settings.fig_name}{settings.fig_name_ext}" + f"_degree{W.shape[0]}",
            )
        return fig, ax
        ...
        for i in range(nRows):
            for j in range(nCols):
                ...
                self._update_axes_labels(
                    ax[i, j],
                    color_thresholds.W_unit_to_N_unit(settings.ticks) * num_grid_fine,
                    settings.ticks,
                )
